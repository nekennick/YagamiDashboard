...
export default async function ProductDetailPage({ params, searchParams }: ProductDetailPageProps) {
  ...
  try {
    ...
    const [salesRows, recentItems, inventoryRows] = await Promise.all([
      getProductBranchMonthlyRows({
        productId,
    ...
    const invoiceCount = new Set(recentItems.map((item) => item.invoiceId)).size;
    const customerCount = new Set(recentItems.flatMap((item) => (item.invoice.customer?.id ? [item.invoice.customer.id] : []))).size;
    ...
    return (
      <div className="space-y-6">
        ...
        <MotionMetricGrid className="md:grid-cols-4">
          ...
          <MetricCard label="Hóa đơn gần đây" value={formatNumber(invoiceCount)} />
          <MetricCard label="Khách đã mua" value={formatNumber(customerCount)} />
        </MotionMetricGrid>
        ...
        <div className="grid gap-4 xl:grid-cols-[0.85fr_1.15fr]">
          ...
          <AnimatedPanel delay={0.14}>
            <Card className="h-full shadow-sm transition-shadow duration-200 hover:shadow-md">
              <CardHeader>
                <CardTitle>Hóa đơn liên quan gần đây</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="overflow-x-auto">
                      ...
                      {recentItems.length === 0 ? (
                        <tr>
                          <td className="px-3 py-8 text-center text-slate-500" colSpan={6}>
                            Không có hóa đơn liên quan trong kỳ.
                          </td>
                        </tr>
                      ) : (
                        recentItems.map((item, index) => (
                          <AnimatedTableRow key={item.id} className="border-b last:border-0" delay={Math.min(index, 12) * 0.015}>
                            ...
                            <td className="px-3 py-2">{item.invoice.customer?.name ?? "Khách lẻ"}</td>
